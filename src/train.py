        ...
        if epoch >= freeze_bn_epoch:
            ...
            for module in model.modules():
                ...
                if isinstance(module, nn.BatchNorm2d):
                    # 将其设置为评估模式
                    # 这会锁定它的 running_mean 和 running_var
                    module.eval() 
        # --------------------------------------------------------

        
        # 正常进行训练
        # 注意：即使 BN 层是 eval() 模式，模型的其他部分（如 Conv, Head）
        # 仍然在 model.train() 的控制下，是可训练的。
        model.train() # 确保其他层是训练模式
        
        # --- 再次确保 BN 层被冻结 ---
        # model.train() 会把所有层都设为 train 模式，我们需要再次把 BN 设回 eval
        if epoch >= freeze_bn_epoch:
             for module in model.modules():
                if isinstance(module, nn.BatchNorm2d):
                    module.eval()
