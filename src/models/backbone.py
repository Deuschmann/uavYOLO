...
class CSPBlock(nn.Module):
    ...
    def __init__(
        ...
        blocks = []
        if use_depthwise:
            # Use IRDCB blocks (expand -> two depthwise convs -> project)
            for _ in range(num_blocks):
                blocks.append(nn.Sequential(IRDCB(hidden_channels, hidden_channels, stride=1, expand_ratio=2.0)))
        else:
            ConvBlock = ConvBNSiLU
            for _ in range(num_blocks):
                blocks.append(
                    nn.Sequential(
